...
const defaultRegex: DomainRegex = {
  domainName: "(?:Domain Name|Domain|domain name|domain): *(.+)",
  registrar:
    "(?:Registrar|Sponsoring Registrar|registrar|sponsoring registrar): *(.+)",
  updatedDate:
    "(?:Updated Date|Last Updated On|updated date|last updated on|changed|Last Modified|Last Update Time)[.:]* *(.+)",
  creationDate:
    "(?:Creat(?:ed|ion) Date|Created On|created|creation date|Registered on)[.:]* *(.+)",
  expirationDate:
    "(?:Expir\\w+ Date|Expires On|expires|expiration date|Expiry date)[.:]* *(.+)",
  status: "(?:Status|Domain Status|status|domain status):\\s*(.+)\\s*\\n",
  nameServers:
    "(?:Name Server|Name Servers|name server|name servers|nserver): *(.+)",
  dateFormat: [
  ...
  notFound:
    "(?:No match|Domain not found|NOT FOUND|No entries found|Status: free|Status: AVAILABLE|is available for registration|The queried object does not exist|No match found|No Data Found)",
  unknownTLD: true,
};
