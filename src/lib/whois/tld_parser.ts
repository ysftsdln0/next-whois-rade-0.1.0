...
export function parseWhoisData(rawData: string, domain: string) {
  ...
  if (filterRegex.expirationDate) {
    ...
    if (match) {
      ...
      const expirationDate = moment(dateStr, filterRegex.dateFormat).toJSON();
  ...
  if (filterRegex.creationDate) {
    ...
    if (match) {
      const dateStr = match[1].trim().replace(/\.$/, "");
      const creationDate = moment(dateStr, filterRegex.dateFormat).toJSON();
  ...
  if (filterRegex.updatedDate) {
    ...
    if (match) {
      const dateStr = match[1].trim().replace(/\.$/, "");
      const updatedDate = moment(dateStr, filterRegex.dateFormat).toJSON();
